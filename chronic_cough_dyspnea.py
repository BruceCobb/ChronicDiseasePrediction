...
# （糖尿病）正例的比例!
manbing_Positive = manbing.where(manbing.drug_3 == '1').count()
# （糖尿病）负例的比例!
manbing_Nagtive = manbing.where(manbing.drug_3 == '0').count()

print("正例占比重为{}".format(manbing_Positive/manbing.count()))# 0.5155441110957494
print("负例占比重为{}".format(manbing_Nagtive/manbing.count()))# 0.48445588890425056

# 根据给定的比例进行随机抽样（正例92%和负例8%）并返回一个新的DataFrame（？？？）
manbing = manbing.sampleBy("drug_3", fractions={'0': 20000/manbing_Nagtive, '1': 20000/manbing_Positive}, seed=0)
# 从源数据中指定（糖尿病）标签列
manbing = manbing.select("erp_code", "d_func", col("drug_3").cast("int").alias('label'))
...
dt_model.write().overwrite().save("./models/cough_dyspnea_dtmodel/")
sameDTModel = PipelineModel.load("./models/cough_dyspnea_dtmodel/")
...
"""
auc=0.965883047378953
acc=0.920577018736528
"""

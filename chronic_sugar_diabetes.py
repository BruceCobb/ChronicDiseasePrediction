...
# （糖尿病）正例的比例!
manbing_Positive = manbing.where(manbing.drug_2 == '1').count()
# （糖尿病）负例的比例!
manbing_Nagtive = manbing.where(manbing.drug_2 == '0').count()

print("正例占比重为{}".format(manbing_Positive/manbing.count()))# 0.07672972640828303
print("负例占比重为{}".format(manbing_Nagtive/manbing.count()))# 0.923270273591717

# 根据给定的比例进行随机抽样（正例92%和负例8%）并返回一个新的DataFrame（？？？）
manbing = manbing.sampleBy("drug_2", fractions={'0': 20000/manbing_Nagtive, '1': 20000/manbing_Positive}, seed=0)
# 从源数据中指定（糖尿病）标签列
manbing = manbing.select("erp_code", "d_func", col("drug_2").cast("int").alias('label'))
...
# 明确机器学习工作流并开始工作
cv_pipeline = Pipeline(stages=[assembler, cv])  # （？？?assembler）
...
print("慢病的数据量为:{}".format(manbing.count()))

bestModel.write().overwrite().save('./models/hypertension_bestmodel/')
end=time.time()
...
dt_pipeline = Pipeline(stages=[assembler, bestDt])  # (???assembler)
dt_model = dt_pipeline.fit(manbing)
dt_model.write().overwrite().save("./models/suger_diabetes_dtmodel/")
sameDTModel = PipelineModel.load("./models/suger_diabetes_dtmodel/")
...
"""
auc=0.9712043389364263
acc=0.9379036264282166
"""

...
# （糖尿病）正例的比例!
manbing_Positive = manbing.where(manbing.drug_4 == '1').count()
# （糖尿病）负例的比例!
manbing_Nagtive = manbing.where(manbing.drug_4 == '0').count()

print("正例占比重为{}".format(manbing_Positive/manbing.count()))# 0.7074884403275112
print("负例占比重为{}".format(manbing_Nagtive/manbing.count()))# 0.29251155967248876

# 根据给定的比例进行随机抽样（正例92%和负例8%）并返回一个新的DataFrame（？？？）
manbing = manbing.sampleBy("drug_4", fractions={'0': 20000/manbing_Nagtive, '1': 20000/manbing_Positive}, seed=0)
# 从源数据中指定（糖尿病）标签列
manbing = manbing.select("erp_code", "d_func", col("drug_4").cast("int").alias('label'))
...
dt_model.write().overwrite().save("./models/gastrointestinal_dtmodel/")
sameDTModel = PipelineModel.load("./models/gastrointestinal_dtmodel/")
...
"""
auc=0.9823213034242594
acc=0.9577770360480641
